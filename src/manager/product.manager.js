...
export class ProductManager {
    ...
    try {
        ...
        // Validar si el campo "code" está repetido.
    ...
    // Elimina un producto por su ID. Aqui creo un nuevp array sin el producto cuyo ID coincida con el que pasamos por parametro.
        async deleteProduct(idProduct) {
        try {
        const products = await this.getProducts();
        if (products.length < 0) return false;
        const updatedArray = products.filter(product => product.id !== idProduct);
        // Sobreescribimos el archivo .json con el nuevo array.
